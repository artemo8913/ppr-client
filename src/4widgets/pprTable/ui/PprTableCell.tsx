...
import { pprFieldValidator, IPprData, IPprTableSettingsContext, TPprDataWorkId, TTransfer } from "@/2entities/ppr";
...
function getValue(pprData: IPprData, field: keyof IPprData, isCorrectedView: boolean) {
  if (pprFieldValidator.isPlanWork(field) || pprFieldValidator.isPlanTime(field)) {
    if (isCorrectedView) {
...
export const PprTableCell: FC<IPprTableCellProps> = ({
  ...
  const isPlanWorkPeriodField = pprFieldValidator.isPlanWork(field);
  const isPlanTimePeriodField = pprFieldValidator.isPlanTime(field);
  const isPlanOrFactWorkPeriodField = pprFieldValidator.isPlanOrFactWork(field);
  ...
  const isPlanTimeField = pprFieldValidator.isWorkOrTime(field);
