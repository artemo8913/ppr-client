...
export const PprTable: FC<IPprTableProps> = ({}) => {
  ...
  const { basicColumns, timePeriods, planFactColumns, monthColSpan } =
    useCreateColumns();

  const pprSettings = usePprTableSettings();

  const pprYearStatus: TYearPprStatus = ppr?.status || "template";
  const pprMonthsStatuses: TAllMonthStatuses | null =
    ppr?.months_statuses || null;
          ...
          <tr key={pprData.id}>
            {basicColumns.concat(planFactColumns).map((field) => (
              <PprTableCellMemo
                ...
                getWorkCorrection={getWorkCorrection}
