import { ITitle } from "../model/pprSchema";
...
export default function Title(props: ITitle) {
  ...
  const infoColumnsTitles = infoColumnsList.map((col, index) => {
    const rowSpan = colSett[col].titleRowSpan;
    const colSpan = colSett[col].titleColSpan;
    const widthPercent = colSett[col].widthPercent;
    colCount++;
    return (
      <Cell key={index} widthPercent={widthPercent} vText rowSpan={rowSpan} colSpan={colSpan}>
        {colSett[col].title}
      </Cell>
    );
  });
