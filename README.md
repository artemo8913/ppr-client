...
1. !!!Тормозит процесс расчета ППРа (нужно оптимизировать перерасчеты). И при этом я не считаю сумму объемов, не считаю человеко часы! Очень долго происходит перерисовка таблицы целиком после изменения контекста
2. Хорошенько обдумать статус ППРа "plan_on_correction". Сейчас это просто промежуточное звено, обозначающее, что ППР отклонили. Можно же сразу отклонить ППР и использовать для этого статуса иное понятие. Как раз таки для корректировки, для изменения.
3. Проверка введенных данных в целом по ППРу
4. Если настой часов запланированных работ больше или меньше на 1, чем имеющийся настой часов, то закрасить цифру с суммарными запланированными человеко-часами
...
7. Экспорт в excel или же pdf без рамок и т.д. и т.п. для печать ЭУ-132, ЭУ-99 и рапортов
8. Формировать рапорт не параллельно, а нажатием на кнопку "Сформировать рапорт"
9. В режиме "Исходный ППР" нельзя ничего редактировать (потому что там исходный. Данные правятся, но это может запутать)
10. Фильтр и поиск по ППРам (только утвержденные, год, цех, дистанция, дирекция и т.д. и т.п.)
11. Статистические таблицы (например, таблица 2 ЭУ-132) и диаграммы в отчетных формах
12. Чел.-ч людей (по норме и по табелю массово заоплнять)
