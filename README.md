...
17. Надо оптимизировать прорисовку стрелок на ППРе (возможно вынести контекст за пределеы arrow conteiner), также нужно минимизировать перерисовки панели управления ППРа, её кнопок
18. При добавлении строки перерисовывается большая часть таблицы, так как в ячейки таблицы передаются индексы (номера) строк, что приводит к перерисовкам. Они нужны для добавления/копирования новых работ. Нужно изменить rowIndex на idObject в функциях ppr provider
